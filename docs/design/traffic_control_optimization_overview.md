...
| 策略类型 | 英文名称 | SUMO实现 | 优先级 |
|---------|---------|---------|-------|
| 可变限速 | Variable Speed Sign (VSS) | `<variableSpeedSign>` | P0 |
| 动态硬路肩 | Dynamic Hard Shoulder (DHS) | lane权限修改 | P0 |
| 货车限行 | Truck Ban (TEC-TB) | `<closingReroute>` + vClass | P1 |
| 入口关闭 | Entrance Closure (TEC-EC) | `<closingReroute>` | P1 |
