...
- 基准方案创建幂等性（重复调用ensure_baseline_plan_exists不会出错）
- 启动失败不影响应用（基准方案创建失败仅记录日志）

### 数据完整性

- baseline_plan目录结构完整
- strategy_ids始终为空数组
- control.add.xml始终为空<additional/>

### 一致性

- 所有case的批量仿真引用同一baseline_plan
- baseline_plan的plan_id固定为"baseline_plan"
...
- ensure_baseline_plan_exists()函数
  - 首次创建
  - 重复调用（幂等性）
  - 异常处理
- 基准方案删除保护
