# Spec Delta: Event Scenario Generation

**Capability**: `event-scenario-generation`

**Change**: Add new capability for generating SUMO simulation configurations from event data
...
**Required Capabilities** (existing):

- `strategy-templates`: Provides control strategy XML generation via `additional_generator.py`
- `plan-management`: Provides control plan creation patterns

**Integrates With** (existing):

- `batch-simulation-management`: Generated scenarios can be applied to batch simulations
- Frontend scenario browser: Loads `scenario_index.json` for display

**Data Dependencies**:

- `events/all_extracted_events.csv`: Source event data with spatial matching
- `templates/network_files/sichuan202508v7.net.xml`: SUMO network for lane resolution
- `templates/config_templates/event_strategy_mapping.json`: Default strategy parameters

---

## Success Criteria

- [ ] Generate valid SUMO `.add.xml` files for 18+ scenarios
- [ ] All XML files pass SUMO schema validation
- [ ] Batch generation completes in < 5 minutes for 30 events
- [ ] Zero manual XML editing required for generated scenarios
- [ ] Scenarios run successfully in SUMO without errors
- [ ] Scenario browser displays all generated scenarios correctly

---

## Notes

**Phase 1 Scope**: Implements accident event injection (`closedLane`) and congestion placeholder. Other event types (road control, geological disasters, breakdowns, weather) deferred to Phase 2.

**Extensibility**: Strategy pattern allows adding new event types by implementing additional `EventInjector` subclasses without modifying existing code.

**Validation Strategy**: Three-level validation (XML syntax → SUMO schema → scenario consistency) ensures generated files are simulation-ready.
