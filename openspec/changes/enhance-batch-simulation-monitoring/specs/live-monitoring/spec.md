# 规格：实时仿真状态监控 (Live Monitoring)

**能力ID**: live-monitoring
**关联变更**: enhance-batch-simulation-monitoring
**版本**: 1.0
...
- 进度查询API响应时间：
  - 缓存命中：<50ms
  - 缓存未命中：<200ms（60任务场景）
- summary.xml增量解析时间：<10ms/文件
- 内存缓存大小：<100MB（100个批次）
- 缓存TTL：5秒（可配置）
- 前端轮询间隔：10秒（从2秒调整）
- live_time_series计算时间：<50ms（60任务场景）

### 可靠性

- summary.xml解析失败不影响其他任务的状态查询
- 文件不存在时优雅降级，显示"初始化中"
- 时间估算失败时显示"计算中"，不报错
...
- 支持不同仿真时长的时间估算：
  - 短时仿真：1-2小时（简单管控策略）
  - 中等仿真：2-6小时（中等复杂度管控）
  - 长时仿真：6-24小时（复杂管控策略，多策略组合）
- 支持未来扩展更多指标（内存使用、CPU负载）
...
- `_extract_summary_last_step()`: 测试增量解析逻辑
- `_estimate_task_remaining_time()`: 测试剩余时间计算公式
- `_estimate_batch_remaining_time()`: 测试批次级估算逻辑
- 边界条件：
  - summary.xml不存在
  - summary.xml为空
  - current_step = 0
...
- 60任务批次进度查询（缓存命中 vs 未命中）
- 频繁轮询（每2秒）的性能影响
- 大文件summary.xml（>10MB）增量解析性能
