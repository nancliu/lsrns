# Specification: Batch Simulation Vehicle Count Chart Fixes

**Capability**: `batch-simulation-charts`
...
Users monitoring batch simulations need clear, stable visualization of vehicle count time series data with proper time scale references. Current issues prevent accurate analysis:

1. **X-axis confusion**: HH:MM format loses seconds precision and doesn't match API `time_points` values
2. **Visual instability**: Chart dimensions fluctuate during updates, disrupting focus and causing UI flicker
3. **Performance overhead**: Complete chart rebuild on each poll creates CPU overhead and data loss risk
4. **Data source ambiguity**: Unclear whether data comes from `live_curve_cache.json` or `progress.json` complicates debugging

This specification unifies the design and implementation approach for all four fixes.
...
*(None - this change does not remove existing requirements)*
...
1. Phase 1: X-axis format + container stability (frontend CSS/JS)
2. Phase 2: Incremental updates (frontend refactor)
3. Phase 3: Data source documentation + verification (backend comments/logging)
4. Phase 4: Testing & validation (manual tests across all scenarios)

**No Breaking Changes**: All modifications are frontend/documentation only with zero API changes or schema modifications.

**Browser Compatibility**: CSS `aspect-ratio` supported in all modern browsers (Chrome 88+, Firefox 89+, Safari 15+, Edge 88+).
